...
DataNodeClient::DataNodeClient(shared_ptr<Channel> channel, int node_id) : stub_(DataNodeSer::NewStub(channel)){
    this->node_id = node_id;
}

long long DataNodeClient::getFileSize(const string & name) {
...
// 创建空文件
int DataNodeClient::addEmptyFile(string name, time_t mtime, time_t atime, time_t ctime) {
...
// 创建含内容的文件
int DataNodeClient::addNewFile(string name, const char * content, long long size, time_t mtime, time_t atime, time_t ctime) {
...
int DataNodeClient::delFile(string name) {
    FileNameRequest request;
    request.set_name(name);
    IntReply reply;
    ClientContext context;
...
int DataNodeClient::delDir(string prefix) {
...
int DataNodeClient::writeToFile(string name, const char * content, long long size) {
...
int DataNodeClient::readFromFile(string name, char * buffer, long long size) {
    ReadRequest request;
    request.set_name(name);
    request.set_size(size);
    ReadReply reply;
...
int DataNodeClient::addFileSeg(string name, const char * content, int size, int seg) {
...
int DataNodeClient::getFileSeg(string name, char * buffer, int size, int seg) {
...
vector<string> DataNodeClient::showChildren(string prefix) {
    PrefixRequest request;
    request.set_prefix(prefix);
    ChildrenReply reply;
    ClientContext context;
...
vector<string> DataNodeClient::showAllChildren() {
...
long long DataNodeClient::getSpaceSize() const
{
    EmptyRequest request;
    LongReply reply;
    ClientContext context;
...
int DataNodeClient::getNodeId()
{
    return node_id;
}

bool DataNodeClient::operator== (const int &other_node_id) const
{
    return this->node_id == other_node_id;
}

bool DataNodeClient::operator< (const DataNodeClient &other_node) const
{
